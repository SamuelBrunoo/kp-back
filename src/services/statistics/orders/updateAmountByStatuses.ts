...
export const updateAmountByStatuses = async (
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (doc.exists()) {
        ...
        const newCurrentStatusAmount = data.amountByStatus[currentStatus] - 1
        const newTargetStatusAmount = data.amountByStatus[targetStatus] + 1
        ...
        const newData: TStatisticsOrder = {
          ...
          amountByStatus: {
            ...data.amountByStatus,
            [currentStatus]:
              newCurrentStatusAmount < 0 ? 0 : newCurrentStatusAmount,
            [targetStatus]:
              newTargetStatusAmount < 0 ? 0 : newTargetStatusAmount,
          },
        }
